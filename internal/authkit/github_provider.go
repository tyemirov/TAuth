...
func (provider *GitHubProvider) identity(ctx context.Context, config tenants.GitHubOAuth, transaction githubTransaction, code string) (githubAuthentication, error) {
	...
	if err != nil {
		return githubAuthentication{}, ErrGitHubProviderRejected
	}
	...
	for _, candidate := range emails {
		...
		if !candidate.Verified {
			return githubAuthentication{}, ErrGitHubEmailMissing
		}
	...
	return githubAuthentication{identity: AccountProviderIdentity{Provider: accountProviderGitHub, Subject: string(user.ID), UserEmail: email, DisplayName: user.Name, AvatarURL: user.AvatarURL}, credential: GitHubCredential{GitHubID: string(user.ID), AccessToken: token.AccessToken, TokenType: token.TokenType, Scope: config.Scopes()}}, nil
}
