...
To restrict sign-ins, set `allowed_users` on a tenant; when present, only those email addresses are permitted to log in.
...
- **403 from `/auth/google` with `user_not_allowed`** – The email is not listed under the tenant’s `allowed_users` allowlist.
