...
- [ ] [TA-500] (P1) Build full account management for first-party email/password identities.
  Planning only. The existing password provider is login-only: configured bcrypt credentials authenticate through `/auth/password/login`, then TAuth mints the standard session and refresh cookies. Full account management must extend that current contract without duplicating the provider path or changing TAuth into a third-party authorization service.
