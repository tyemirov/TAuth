...
- TA-107: `auth-client.js` now derives its base URL from the script origin and exposes `getAuthEndpoints()` so consumers can reuse core `/auth/*` + `/me` routes without manual wiring.
