...
- TA-109: Generalized preflight reporting with `tools/utils/preflight`, added a Viper-based adapter for YAML + env bindings, and refactored TAuth preflight to reuse the shared schema.

### Improvements ⚙️
- TA-411: Moved the preflight package to `tools/utils/preflight` to make it reusable outside the TAuth module.
