...
- [x] [TA-209] Add a footer from mpr-ui to the demo.html — Demo now mounts the shared `MPRUI.mprFooter` Alpine factory with CDN Alpine bootstrap, serves `mpr-ui.js` locally via `/static/mpr-ui.js` to avoid CDN MIME mismatches, and ships both string-level and browser automation coverage to keep the footer visible.
