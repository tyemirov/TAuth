...
- [x] [TA-105] Update `web/auth-client.js`, README, ARCHITECTURE, and usage docs to explain how front-ends select a tenant (document host mapping, new `initAuthClient({ tenantId })` option for shared hosts), and add integration tests that exercise two tenants end-to-end. — `auth-client.js` now accepts `tenantId`, all requests emit `X-TAuth-Tenant`, docs describe the workflow, and HTTP tests cover multi-tenant header overrides.
- [ ] [TA-106] Unify configuration by requiring all deployments (single or multiple tenants) to use a tenants JSON file; CLI/env flags should load that file, support exactly one configuration path, and ensure the UX clearly documents the process.
