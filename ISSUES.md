...
- [x] [TA-307] Harden Google Sign-In flow — Demo now reads base URLs from `/demo/config.js`, reinitialises Google Identity with freshly issued nonces, surfaces auth errors inline, avoids redundant `/me` fetches, and documents the CORS/SameSite requirement for cross-origin cookies.
...
- [x] [TA-306] Restore demo login after TA-305 regression — Consolidated session handling behind a single `runAuthClient` helper so the demo logs events without duplicating `initAuthClient`, and confirmed sign-in works end-to-end.
```js
