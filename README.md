...
- Front-ends that share a single host can opt into an explicit tenant selection by passing `tenantId` to `initAuthClient({ ... })`. The helper automatically adds the `X-TAuth-Tenant` header to every request, matching the backend override flow described above.
