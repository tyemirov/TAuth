...
- Start the server with `--tenants_file=/path/to/tenants.yaml` (or `APP_TENANTS_FILE`). There is no alternative configuration path; single-tenant and multi-tenant deployments both rely on the same file.
